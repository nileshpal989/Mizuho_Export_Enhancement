...
public partial class IMP_Transactions_TF_IMP_BOE_Checker_View : System.Web.UI.Page
{
    ...
    protected void fillGrid()
    {
        ...
        TF_DATA objData = new TF_DATA();
        DataTable dt = objData.getData("TF_IMP_BOE_GetList_ForChecker", p_search, p_BranchName, p_Year, p_LodgementDate, p_Status);
        if (dt.Rows.Count > 0)
        {
    ...
    protected void GridViewImpEntryList_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            string Collection_Lodgment = "", Sight_Usance = "";
            ...
            {
                string pageurl = "window.location='TF_IMP_BOE_Checker.aspx?mode=Edit&DocNo=" + lblDocumentNo.Text + "&DocType=" + lblDocType.Value.Trim() +
            "&BranchName=" + lblBranchName.Text + "&DocYear=" + txtYear.Text.Trim() + "&lblDocument_Scrutiny=" + lblDocument_Scrutiny.Value +
            "&flcIlcType=" + lblForeignLocal.Value.Trim() + "&Collection_Lodgment=" + Collection_Lodgment + "&Sight_Usance=" + Sight_Usance + "&Status=" + Status.Text + "'";
                if (i != 8)
