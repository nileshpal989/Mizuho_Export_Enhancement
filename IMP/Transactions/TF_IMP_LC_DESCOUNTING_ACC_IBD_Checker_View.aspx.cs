...
public partial class IMP_Transactions_TF_IMP_LC_DESCOUNTING_ACC_IBD_Checker_View : System.Web.UI.Page
{
    ...
    protected void fillGrid()
    {
        ...
        TF_DATA objData = new TF_DATA();
        DataTable dt = objData.getData("TF_IMP_LCdiscount_ACC_IBD_GetList_Checker", p_search, p_BranchName, p_Year, p_IBDDate, p_Status);
        if (dt.Rows.Count > 0)
        {
    ...
    protected void GridViewLCdescounting_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            {
                path = "TF_IMP_LC_DESCOUNTING_ACC_IBD_Checker.aspx";
            }
            else
            {
                path = "TF_IMP_LC_DESCOUNTING_DISCREPANT_ACC_IBD_Checker.aspx";
            }
            int i = 0;
            ...
            "&DocScrutiny=" + lblDocScrutiny.Text.Trim() + "&IBDDocument_No=" + lblIBDDocNo.Text.Trim() + "&IBDExtnDocNo=" + lblIBDExtnDocNo.Text + "&IBDExtn=" + lblIBDExtn.Text + "&Status=" + Status + "'";
                if (i != 13)
