...
public partial class IMP_Transactions_TF_IMP_LC_DESCOUNTING_DISCREPANT_ACC_IBD_Checker : System.Web.UI.Page
{
    ...
    protected void Fill_Logd_Accept_Details()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            txtSettl_ForBank_Abbr.Visible = true;
            ddl_Settl_ForBank_Abbr.Visible = false;
            ...
            //txtLCNo.Text = dt.Rows[0]["Doc_LC_No"].ToString();                  11/12/2019 FEEDBEACK
            //fillCustomerMasterDetails();
            //txtRiskCust.Text = dt.Rows[0]["Cust_Abbr"].ToString();
            //txtApplNo.Text = dt.Rows[0]["txnappno"].ToString();                11/12/2019 FEEDBEACK
            ...
            txt_R42_Amt_4488.Text = dt.Rows[0]["Bill_Amount"].ToString();
    ...
    protected void Fill_LCDiscountingDetails()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            ddl_Settl_ForBank_Abbr.ClearSelection();
            txtPurpose.Text = dt.Rows[0]["Purpose_Type"].ToString();
            ...
            if (Request.QueryString["BranchName"].Trim() == "Mumbai" && lblDoc_Curr.Text == "INR")
            {
                txtSettl_ForBank_Abbr.Text = dt.Rows[0]["Settl_For_Bank_Abbr"].ToString();
            }
            else
            {
                ddl_Settl_ForBank_Abbr.SelectedValue = dt.Rows[0]["Settl_For_Bank_Abbr"].ToString();
            }
            txtSettl_ForBank_AccCode.Text = dt.Rows[0]["Settl_ForBank_AccCode"].ToString();
            txtSettl_ForBank_AccNo.Text = dt.Rows[0]["Settl_ForBank_AccNo"].ToString();
            ...
            if (dt.Rows[0]["GO2_Bill_Handling_Flag"].ToString() == "Y")
            {
                ...
                txt_GO2_Debit_Code.Text = dt.Rows[0]["GO2_Bill_Handling_Debit_Code"].ToString();
                ...
                txt_GO2_Credit_Code.Text = dt.Rows[0]["GO2_Bill_Handling_Credit_Code"].ToString();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        {
            AR = "A";
            IBDGbaseFileCreation();
            //GOBRFileCreation();
            if (chk_GO1_Flag.Checked == true)
            {
                GO1FileCreation();
            }
            if (hdnIBDExtnFlag.Value != "Y")
            {
                CreateSwiftR42();
            }
    ...
    public void CreateSwiftR42()
    {
        ...
        SqlParameter PDocNo = new SqlParameter("@DocNo", txtDocNo.Text.ToString());
        SqlParameter PIBDDocument_No_Extn = new SqlParameter("@IBDDocument_No_Extn", txtIBDExtnNo.Text.ToString());
        DataTable dt = objData1.getData("TF_IMP_IBD_R42_ExcelFile", PDocNo, PIBDDocument_No_Extn);
        string TodayDate = DateTime.Now.ToString("dd/MM/yyyy");
        string MTodayDate = "";
        if (TodayDate.Contains("/"))
        {
    ...
    public void GO2FileCreation()
    {
        TF_DATA objData1 = new TF_DATA();
        SqlParameter PDocNo = new SqlParameter("@DocNo", txtDocNo.Text.ToString());
        SqlParameter PIBDDocument_No_Extn = new SqlParameter("@IBDDocument_No_Extn", txtIBDExtnNo.Text.ToString());
        DataTable dt = objData1.getData("TF_IMP_LCDiscounting_IBD_ACC_GbaseFileCreation_GO2", PDocNo, PIBDDocument_No_Extn);
        string TodayDate = DateTime.Now.ToString("dd/MM/yyyy");
        string MTodayDate = "";
        if (TodayDate.Contains("/"))
        {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            string _filePath = _directoryPath + "/" + txtIBDDocNo.Text.Trim() + "_GO2" + ".xlsx";
            if (dt.Rows.Count > 0)
            {
                using (XLWorkbook wb = new XLWorkbook())
                {
