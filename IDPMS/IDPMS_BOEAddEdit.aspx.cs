...
public partial class IDPMS_IDPMS_BOEAddEdit : System.Web.UI.Page
{
    string currncy = "" , txtbilln ="";
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (!IsPostBack)
            {
                ...
                btnHelp_DocNo.Attributes.Add("onclick", "return OTT_Help('mouseClick')");
                txtORMNo.Attributes.Add("onkeydown", "return OTT_Help('this');");
                btnBillEntryNo.Attributes.Add("onclick", "return Dump_Help('mouseClick')");
                txtbillno.Attributes.Add("onkeydown", "return Dump_Help('this');");
                btnAddGRPPCustoms.Attributes.Add("onkeydown", "return btnadgrppenter('mouseClick')");
                btnSave.Attributes.Add("onclick", "return ValidateSave();");
               // btnBillEntryNo.Attributes.Add("onkeydown", "return Dump_Help('mouseClick')");
    ...
    {
         currncy = hdnlblboecurrency.Value;
        if (lblcurren.Text != lblboecur.Text)
        {
            if (txtExchangeRate.Text.Trim() == "")
            {
                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "message", "alert('Selected BOE No. Currency is Diffrent ! Please Enter Exchange Rate.');", true);
                txtExchangeRate.Focus();
                return;
            }
        }
        AddNewRowToGrid();
        txtbillno.Text = txtbilln;
        ...
        if (ViewState["CurrentTable"] != null)
        {
            ...
            for (int a = 0; a < dtCurrentTable.Rows.Count; a++)
            {
                if (lblboecur.Text != dtCurrentTable.Rows[a]["Invoice_Currency"].ToString())
                    //if (currncy != dtCurrentTable.Rows[a]["Invoice_Currency"].ToString())
                    {
            ...
            TF_DATA obj = new TF_DATA();
            string query = "TF_Get_BillDetails";
            ...
            SqlParameter P4 = new SqlParameter("@ExchangeRate", txtExchangeRate.Text.Trim());
            SqlParameter P5 = new SqlParameter("@ExchangeRateSign", ddlExchangeRateSign.SelectedValue);
            SqlParameter P6 = new SqlParameter("@ORMCurrency", lblcurren.Text.Trim());

            DataTable dt = obj.getData(query, P1, P2, P3, P4, P5,P6);
            if (dt.Rows.Count > 0)
            {
                for (int s = 0; s < dt.Rows.Count; s++)
                {
                    ...
                    drCurrentRow["Record_inc"] = dt.Rows[s]["Record_inc"].ToString();
                    drCurrentRow["InvoiceAmt"] = dt.Rows[s]["InvoiceAmt"].ToString();
                    drCurrentRow["PayAmt"] = dt.Rows[s]["PayAmt"].ToString();
    ...
    {
        Response.Redirect("IDPMS_BOEAddEdit.aspx?mode=Add", true);
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (result == "Save")
            {
                ...
                SqlParameter Q9 = new SqlParameter("@MenuName", "Bill Of Entry - Payment Settlement Data Entry");
                ...
                Clear();
                txtPartyID.Focus();// Added by Anand 05-01-2024
            }
    ...
    protected void Clear()
    {
        ...
        lblboecur.Text = "";
        lblLink.InnerText = "";
        txtPartyID.Text = "";//Added by Anand 05/01/2024
        txtORMNo.Text = "";//Added by Anand 05/01/2024
        lblCustName.Text = "";//Added by Anand 05/01/2024
        lblOrmAmount.Text = "";//Added by Anand 05/01/2024
    }
    protected void txtbillno_TextChanged(object sender, EventArgs e)
    {
        txtbilln = hdntxtbill.Value;
        TF_DATA obj = new TF_DATA();
        string query = "TF_Selected_BOE_details";
        SqlParameter P1 = new SqlParameter("@BOENO", txtbilln);
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            if (lblcurren.Text != lblboecur.Text)
            {
                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "message", "alert('BOE NO : " + hdntxtbill.Value + " Currency Is Diffrent Than ORM Currency !');", true);//Added by Anand 05-01-2023 txtbillno.Text to hdntxtbill.Value
            }
            if (daydiff >= 180)
            {
                string CustACNo = "";
                ...
                lblLink.Attributes.Add("href", "TF_AddEditPaymentExtn.aspx?CustACNo=" + CustACNo + "&BOENo=" + hdntxtbill.Value + "");//Added by Anand 05-01-2023 txtbillno.Text to hdntxtbill.Value
                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "message", "alert('BOE NO : " + hdntxtbill.Value + " Requires Payment Extension !');", true);//Added by Anand 05-01-2023 txtbillno.Text to hdntxtbill.Value
            }
        }
        else
        {
    ...
    protected void fillgrid()
    {
        TF_DATA obj = new TF_DATA();
        string query = "TF_Get_BillDetails";
        SqlParameter P1 = new SqlParameter("@billno", txtbilln);
        SqlParameter P2 = new SqlParameter("@billdate", txtbilldate.Text);
        SqlParameter P3 = new SqlParameter("@portcode", txtprtcd.Text);
        SqlParameter P4 = new SqlParameter("@ExchangeRate", txtExchangeRate.Text.Trim());
        SqlParameter P5 = new SqlParameter("@ExchangeRateSign", ddlExchangeRateSign.SelectedValue);
    ...
    protected void txtPayAmt_textchange(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < GridViewEDPMS_Bill_Details.Rows.Count; i++)
        {
            if (GridViewEDPMS_Bill_Details.Rows.Count > 0)
            {
                ...
                txtPamt = (TextBox)GridViewEDPMS_Bill_Details.Rows[i].Cells[7].FindControl("txtPayAmt");
    ...
    protected void lblinvcamt_textchange(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < GridViewEDPMS_Bill_Details.Rows.Count; i++)
        {
            if (GridViewEDPMS_Bill_Details.Rows.Count > 0)
            {
                ...
                lblinvcamt = (TextBox)GridViewEDPMS_Bill_Details.Rows[i].Cells[6].FindControl("lblinvcamt");
                CheckBox chkrow = new CheckBox();
                chkrow = (CheckBox)GridViewEDPMS_Bill_Details.Rows[i].FindControl("RowChkAllow");
                if (chkrow.Checked == true)
                {
