...
public partial class Reports_IDPMSReports_View_outwardRemAck : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (Request.QueryString["branchname"] != null)
            {
                ...
                if (Request.QueryString["type"] == "ORM")
                {
                    serverReport.ReportPath = "/Mizuho_TF_UAT_Reports/Mizho_AckOutwardRemittance";
                }
                else if (Request.QueryString["type"] == "closure")
                {
                    serverReport.ReportPath = "/Mizuho_TF_UAT_Reports/TF_AckOutwardRemittance_Closure";
                }
