...
public partial class EXP_EXP_ViewRealisationEntry_Checker : System.Web.UI.Page
{
        ...
        else
        {
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["AR"] != null)
                {
                    ...
                    {
                        if (Request.QueryString["chkIRMCreate"].Trim() == "true")
                        {
                        ...
                        {
                            string script2 = "alert('Record Approved with Trans.Ref.No " + _DocNo + "')";
                            ScriptManager.RegisterClientScriptBlock(this.Page, Page.GetType(), "message", script2, true);
                        }
                    }
    ...
    protected void GridViewEXPDocRealised_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            {
                string pageurl = "window.location='EXP_AddEditFullRealisation_Checker.aspx?mode=edit&DocNo=" + lblDocumentNo.Text.Trim()
    ...
    protected void fillGrid()   
    {
        ...
        string query = "TF_EXPGetRealisationEntryListNew_Checker";
