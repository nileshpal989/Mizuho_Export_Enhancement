...
public partial class EXP_EXP_AddEditFullRealisationPRN_Maker : System.Web.UI.Page
{
    string branchcode = "";
    string DocPrFx = "";
    string SrNo = "";
    ...
    protected void fillDetails(string docno, string SrNo)
    {
        ...
        p2.Value = SrNo;
        string query = "TF_GetRealisedEntryDetailsNew_PeningPRN";
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            hdnRejectReason.Value = dt.Rows[0]["ApprovePRNChecker_Remark"].ToString();
            if (dt.Rows[0]["ApprovePRNChecker_Remark"].ToString() != "")
            {
                lblChecker_Remark.Text = "Checker Remark :- " + dt.Rows[0]["ApprovePRNChecker_Remark"].ToString();
            }
    ...
    {
        Response.Redirect("EXP_ViewRealisationEntry_PRN_Maker.aspx", true);
    }
    ...
    protected void Fillshippingbill()
    {
        ...
        DataTable dt = objsave.getData("TF_EXP_filldetailsShippingbill_PendingPRN", p1, p2, p3);
    ...
    protected void RowEnableshippingbill()
    {
            ...
            for (int i = 0; i < GridViewGRPPCustomsDetails.Rows.Count; i++)
        {
                ...
                TextBox txtFBANK = (TextBox)GridViewGRPPCustomsDetails.Rows[i].FindControl("txtFBANK");
                DropDownList ddlpartfull = (DropDownList)GridViewGRPPCustomsDetails.Rows[i].FindControl("ddlpartfull");
                if (txtsettelementamt.Text != "0.00")
                {
                     CheckBox chkrow = (CheckBox)GridViewGRPPCustomsDetails.Rows[i].FindControl("RowChkAllow");
                     chkrow.Checked = true;
                     chkrow.Enabled = false;
                     txtsettelementamt.Enabled = false;
                     txtFBANK.Enabled = false;
                     ddlpartfull.Enabled = false;
                }
        }
        }
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterClientScriptBlock(this.Page, Page.GetType(), "message", "alert('Please Uncheck Pending Shipping Bill Flag.');", true);
            flag = false;
        }
        if (flag == true)
        {
            ...
            SqlParameter puploadingdate = new SqlParameter("@uploadingdate", _uploadingDate);
            ...
            SqlParameter checkerstatus = new SqlParameter("@ApprovePRN", "C");
            ...
            string query = "TF_UpdateExportRealisationDetails_PendingPRN";
            string _result = "";

            TF_DATA objSave = new TF_DATA();
            TF_DATA objData = new TF_DATA();

            _result = objSave.SaveDeleteData(query, branch, docno, srno, pCheckPendingSB, checkerstatus, puser, puploadingdate);
            ...
            // if (_result == "added")
            {
                ...
                {
                    _script = "window.location='EXP_ViewRealisationEntry_PRN_Maker.aspx?result=" + _result + "&saveType=print&frm=" + txtDateRealised.Text.Trim() + "&rptType=" + "Single" + "&rptFrdocno=" + txtDocNo.Text.Trim() + "&rptTodocno=" + txtDocNo.Text.Trim() + "&rptCode=" + "2" + "&Branch=" + Session["userADCode"].ToString() + "&Report=" + "Export Bill Document Realisation'";
                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "redirect", _script, true);
                }
                else
                {
                    _script = "window.location='EXP_ViewRealisationEntry_PRN_Maker.aspx?result=" + _result + "&saveType=Submit&frm=" + txtDateRealised.Text.Trim() + "&rptType=" + "Single" + "&rptFrdocno=" + txtDocNo.Text.Trim() + "&rptTodocno=" + txtDocNo.Text.Trim() + "&rptCode=" + "2" + "&Branch=" + Session["userADCode"].ToString() + "&Report=" + "Export Bill Document Realisation'";
                    ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "redirect", _script, true);
                }
            }
            else
