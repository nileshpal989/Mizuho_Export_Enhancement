...
public partial class EXP_EXP_AddEditApprovedLodgemetForROD_Maker : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (!IsPostBack)
            {
                ...
                {
                    Response.Redirect("EXP_ViewApprovedLodgemetForROD_Maker.aspx", true);
                }
                else
                {
                    ...
                    switch (txtDocType.Text)
                    {
                        ...
                        case "EB":
                            ...
                            btnTTRefNoList.Enabled = true;
                            //btnFircRefNoList.Enabled = false;
                            txtExchRate.Text = "1";
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (txtBillAmount.Text.Trim() == "" || txtBillAmount.Text.Trim() == "0.00")
        {
        ...
        else
        {
            ...
            if (_proceed1 == true)
            {
                if (_proceed == true)
                {
                    ...
                    SqlParameter pApproveROD = new SqlParameter("@ApproveROD", "C");
                    ...
                    string _query = "TF_EXP_UpdateApprovedLodgementforRODDetails";
                    TF_DATA objSave = new TF_DATA();

                    _result = objSave.SaveDeleteData(_query, bCode, docNumber, pCheckPendingSB, pApproveROD, pUser, pUploadDate);
                    ...
                    else
                    {
                        if (_result == "updated")
                        {
                            if (argument.ToString() == "print")
                            {
                                _script = "window.location='EXP_ViewApprovedLodgemetForROD_Maker.aspx?result=" + _result + "&saveType=print&frm=" + txtDateRcvd.Text.Trim() + "&rptType=" + "Single" + "&rptFrdocno=" + txtDocumentNo.Text.Trim() + "&rptTodocno=" + txtDocumentNo.Text.Trim() + "&rptCode=" + "2" + "&Branch=" + Session["userADCode"].ToString() + "&Report=" + "Export Bill lodgement'";
                                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "redirect", _script, true);
                            }
                            else
                            {
                                _script = "window.location='EXP_ViewApprovedLodgemetForROD_Maker.aspx?result=" + _result + "&saveType=Submit&frm=" + txtDateRcvd.Text.Trim() + "&rptType=" + "Single" + "&rptFrdocno=" + txtDocumentNo.Text.Trim() + "&rptTodocno=" + txtDocumentNo.Text.Trim() + "&rptCode=" + "2" + "&Branch=" + Session["userADCode"].ToString() + "&Report=" + "Export Bill lodgement'";
                                ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "redirect", _script, true);
                            }
                        }
                        else
    ...
    {
       //deleted by anand //11/04/2024 not in use
    }
