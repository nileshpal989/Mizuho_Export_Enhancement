...
public partial class EDPMS_TF_EBRC_ORM_APISTATUS_Fileupload : System.Web.UI.Page
{
    ...
    private void GetExcelSheets(string FilePath, string Extension, string isHDR)
    {
        try
        {
            ...
            string Branch = "", bankuniquetxid = "", ORMIssuedate = "", ORMNo = "", ORMStatus = "", ifsccode = "", adcode = "",
                paymentdate = "", ORNFcc = "", ORNFCCAmt = "", INRpayableAMT = "", exchangerate = "", ieccode = "", panno = "", benefname = "",
                benefcontry = "",purposecode = "", refIRM = "", Push_Status = "",Push_Error = "",Get_Status = "",RecvdRecordCount = "",
                ...
                if (worksheet != null)
                {
                    ...
                    for (int row = 2; row <= endCell.Row; row++)
                    {
                        ...
                        if (!isEmptyRow)
                        {
                            ...
                            string _query = "TF_EBRC_ORM_APISTATUS_Upload";
                            noofrecinexcel++;
                            SRNO.Value = "";
                            ...
                            exchangerate = "";
                            ieccode = worksheet.Cells[row, 11].Text;
                            panno = worksheet.Cells[row, 12].Text;
                            benefname = worksheet.Cells[row, 13].Text;
                            benefcontry = worksheet.Cells[row, 14].Text;
                            purposecode = worksheet.Cells[row, 15].Text;
                            refIRM = worksheet.Cells[row, 16].Text;
                            Push_Status = worksheet.Cells[row, 17].Text;
                            Push_Error = worksheet.Cells[row, 18].Text;
                            Get_Status = worksheet.Cells[row, 19].Text;
                            RecvdRecordCount = worksheet.Cells[row, 20].Text;
                            processedRecordCount = worksheet.Cells[row, 21].Text;
                            irmStatusLst = worksheet.Cells[row, 22].Text;
                            actualdatd = worksheet.Cells[row, 23].Text;
                            EX_Exception = worksheet.Cells[row, 24].Text;
                            ...
                            if (dt2.Rows.Count > 0)
                            {
                                ...
                                result = objSave.SaveDeleteData(_query, BranchCode, SRNO, BANK_UNIQUE_TXID, ORM_ISSUE_DATE, ORMNO, ORMSTATUS, 
                                    IFSCCODE, ADCODE,PAYMENTDATE, ORNFCC, ORNFCCAMT, INRPAYABLEAMT, EXCHANGERATE, IECCODE, PANNO, BENEFNAME, 
                                    BENEFCOUNTRY, PURPOSECODE,REF_IRM,PUSH_STATUS,PUSH_ERROR,GET_STATUS,RECVDRECORDCOUNT,PROCESSEDRECORDCOUNT,
                                    IRMSTATUSLST,ACTUALDATA,EXEXCEPTION,ADD_USER, ADD_DATE);


                                if (result == "Uploaded")
                                {
                                    objSave.SaveDeleteData("TF_EBRC_ORM_UpdateAPIStatus", BANK_UNIQUE_TXID, PUSH_STATUS, GET_STATUS);
                                    dtcount = dtcount + 1;
                                }
                                else
                                {
